...
function OnClick(){
    ...
    let regexp = /.+@\w+\.ru/d;
    if((input_email.value == "" || input_password.value == "") && document.querySelector(".warning-label-container.null-fields")==null){
        let previous_warnings = document.querySelector(".warning-label-container");                   //only 2 types of warnings
        if(previous_warnings != null) document.querySelector("form.form").remove(previous_warnings);  //or just change content in this case
        let warning_label_container = document.createElement("div");
        warning_label_container.classList.add("warning-label-container");
        warning_label_container.classList.add("null-fields");
    ...
    else if(!regexp.test(input_email.value) && document.querySelector(".warning-label-container.wrong-email")==null){
        let previous_warnings = document.querySelector(".warning-label-container");
        if(previous_warnings != null) document.querySelector("form.form").remove(previous_warnings);
        let warning_label_container = document.createElement("div");
        warning_label_container.classList.add("warning-label-container");
        warning_label_container.classList.add("wrong-email");
        let warning_text = document.createElement("span");
        warning_text.classList.add("warning-text");
